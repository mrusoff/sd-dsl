...
(
    ...
    { 
        $current = $this_XsdElement_3.current; 
        afterParserOrEnumRuleCall();
    }
)
;
...
entryRuleXsdElement returns [EObject current=null] 
	:
	{ newCompositeNode(grammarAccess.getXsdElementRule()); }
	 iv_ruleXsdElement=ruleXsdElement 
	 { $current=$iv_ruleXsdElement.current; } 
	 EOF 
;

// Rule XsdElement
ruleXsdElement returns [EObject current=null] 
    @init { enterRule(); 
    }
    @after { leaveRule(); }:
(	otherlv_0='element' 
